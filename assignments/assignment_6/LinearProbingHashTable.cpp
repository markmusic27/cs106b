// This file defines all of the methods in the LinearProbingHashTable class using the LinearProbingHashTable:: operator as well as defines the tests!

#include "LinearProbingHashTable.h"
using namespace std;

// This function is a constructor and it initializes all pointers and class state
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    // Store the hash function
    this->hashFn = hashFn;

    // Get the number of slots
    kNumSlots = hashFn.numSlots();
    numElems = 0;

    // Allocate the array dynamically
    elems = new Slot[kNumSlots];
...
// This function is called a destructor and eliminates all state
LinearProbingHashTable::~LinearProbingHashTable() {
    delete[] elems;
}

// The function merely returns the amount of elements in the hash table
int LinearProbingHashTable::size() const {
    return numElems;
}

// The function merely checks if number of elements equals zero
bool LinearProbingHashTable::isEmpty() const {
    return numElems == 0;
}

// This function walks through the hash array and tries to insert the element closest to its home determined by its hash.
bool LinearProbingHashTable::insert(const string& elem) {
    // Check if the element is already in the array!
    if (contains(elem)) {
        return false; // Womp womp, no duplicates. Be creative, geez.
    }

    int slot = hashFn(elem);
    int checkedSlots = 0;
    // Current step as we walk forward
    int step = slot;

    while (checkedSlots < kNumSlots) {

        // Insert items into empty slots or tombstones
        if (elems[step].type == SlotType::EMPTY || elems[step].type == SlotType::TOMBSTONE){
            numElems++;
            elems[step].type = SlotType::FILLED;
            elems[step].value = elem;

            return true;
        }

        // Move to the next slot (with wrap-around)
        step = (step + 1) % kNumSlots;
        checkedSlots++;
    }
...
// Itterates through elem until the item is found by finding the first blank space or the element itself (skips through tombstones)
bool LinearProbingHashTable::contains(const string& elem) const {
    // Compute the hash of the element to get the slot
    int slot = hashFn(elem);
    ...
    while (checkedSlots < kNumSlots) {
        // If slot is EMPTY, the key isn't in the table
        if (elems[step].type == SlotType::EMPTY) {
            return false;
        }

        // If slot contains the key and is FILLED (shouldn't matter but still!), we've found it!
        if (elems[step].type == SlotType::FILLED && elems[step].value == elem) {
            return true;
        }

        // Move to the next slot (with wrap-around... where modulus operator comes in!)
        step = (step + 1) % kNumSlots;
        checkedSlots++;
    }

    // Looped around and didn't find value
    return false;
}

// Removes elements by replacing removed elements with a tombstone. If the element is not in the array, then we can tell by finding a blank space before the element.
bool LinearProbingHashTable::remove(const string& elem) {
    /*
     * Don't need to check if the array contains it with the previous function
     * because it is not necessary since it will be achieved by checking for empty slots
    */
    ...
    while (checkedSlots < kNumSlots) {
        ...
        // If slot contains the key and is FILLED (shouldn't matter but still!), we've found it!
        if (elems[step].type == SlotType::FILLED && elems[step].value == elem) {
            elems[step].type = SlotType::TOMBSTONE;
            elems[step].value = "";
