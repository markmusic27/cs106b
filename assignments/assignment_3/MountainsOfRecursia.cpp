...
Vector<Point> makeMountainRange(const Point& left,
    ...
    if (amplitude < 0){
        error("The amplitude value cannot be negative");
    }

    if (decayRate < 0 || decayRate > 1){
        error("Invalid value for decayRate. Cannot be greater than 1 or negative.");
    }
    ...
    int midpointY = (left.y + right.y)/2;
    int noise = randomInteger(-amplitude, amplitude);

    // Calls recursive makeMountainRange for subsets of range
    Point midpoint = { midpointX, midpointY+noise };
    Vector<Point> leftRange = makeMountainRange(left, midpoint, amplitude*decayRate, decayRate);

    Vector<Point> rightRange = makeMountainRange(midpoint, right, amplitude*decayRate, decayRate);

    // Remove first index in rightRange to prevent there being a copy of midpoint
    rightRange.remove(0);
