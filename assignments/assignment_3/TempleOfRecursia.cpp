...
Rectangle drawRectangle(double fHeight, double fWidth, Point origin, Rectangle bounds);

Vector<Rectangle> makeTemple(const Rectangle& bounds, const TempleParameters& params) {

    // Add edge cases for order
    if (params.order < 0){
    ...
    // Create temple base
    Rectangle base = drawRectangle(params.baseHeight, params.baseWidth, origin, bounds);
    canvas.add(base);

    // Create template column
    Rectangle column = drawRectangle(params.columnHeight, params.columnWidth, {origin.x, origin.y-base.height}, bounds);
    canvas.add(column);

    // Add child temples above base and overlaying column
    int cTempHeight = bounds.height * params.smallTempleHeight;
    int cTempWidth = bounds.width * params.smallTempleWidth;
    Point cTempOrigin = {(bounds.width - base.width)/2+origin.x, origin.y-base.height};
    int spaceBetweenChildTemples = (base.width - (params.numSmallTemples*cTempWidth))/(params.numSmallTemples-1);
    Vector<Rectangle> childTemples = {};

    // Itterates sufficiently to add child temples and spacers
    for (int i = 0; i < params.numSmallTemples; i++){
        int cTempX = cTempOrigin.x + (cTempWidth+spaceBetweenChildTemples)*i;
        int cTempY = cTempOrigin.y + bounds.height - cTempHeight;

        childTemples.add({cTempX, cTempY, cTempWidth, cTempHeight});
    }

    // Recursive call to generate upper bounds

    Point childOrigin = {origin.x,origin.y-base.height-column.height};
    Rectangle childBounds = drawRectangle(params.upperTempleHeight, params.columnWidth, childOrigin, bounds);

    TempleParameters newParams = params;
    newParams.order = newParams.order - 1;

    Vector<Rectangle> children = makeTemple(childBounds, newParams);

    return canvas + children + childTemples;
}

Rectangle drawRectangle(double fHeight, double fWidth, Point origin, Rectangle bounds){
    int width = fWidth * bounds.width;
    int height = fHeight * bounds.height;
    int rectY = origin.y + bounds.height - height;
    int rectX = ((bounds.width - width)/2)+origin.x;
...
PROVIDED_TEST("Milestone Four: Draws four smaller towers of the correct size.") {
    ...
    /* Order-1 (Leftmost Small) Base. */
    EXPECT_EQUAL(temple[4], { 10000768, 20000032, 128, 64 });
}
