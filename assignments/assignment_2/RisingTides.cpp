...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    // Creates a new flood map with dimensions of terrain (all set to false initially)
    Grid<bool> floodMap(terrain.numRows(), terrain.numCols(), false);
    Queue<GridLocation> path;

    for (const GridLocation& location : sources) {
        // Adds all flooded areas at or below terrain level to queue
        int x = location.col;
        int y = location.row;

        if (terrain.inBounds(y, x) && height >= terrain[y][x]) {
            floodMap[y][x] = true;
    ...
    while (!path.isEmpty()) {
        GridLocation current = path.dequeue();

        Vector<GridLocation> neighbors;
        int row = current.row;
        int col = current.col;

        // Check neighbors in the four cardinal directions
        for (int x = -1; x <= 1; x += 2) {
            if (terrain.inBounds(row, col + x)) {
                neighbors.add(GridLocation(row, col + x));
            }
        }

        for (int y = -1; y <= 1; y += 2) {
            if (terrain.inBounds(row + y, col)) {
                neighbors.add(GridLocation(row + y, col));
            }
        }

        // Flood if necessary
        for (const GridLocation& n : neighbors) {
            if (!floodMap[n.row][n.col] && height >= terrain[n.row][n.col]) {
                path.enqueue(n);
                floodMap[n.row][n.col] = true;
            }
        }
    }
...
// Student test
STUDENT_TEST("Water can flow with extreme height differentials") {
    Grid<double> world = {
        { -10000, -1000, 1000 },
        { -1000, 1000, -1000 },
        { 1000, -1000, -10000 }
