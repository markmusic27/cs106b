...
void addGramToMap(Map<string, double>& map, string gram);

Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0){
          error("kGramLength is less than or equal to zero. Not permitted value.");
    }
    ...
    for (int i = 0; i < str.length(); i++){
        int distToEnd = i + kGramLength - 1;

        if (distToEnd < str.length()){
            string gram = str.substr(i, distToEnd - i + 1);
            addGramToMap(kGrams, gram);
        }
    }
...
// Helper function that adds gram to map or increases frequency
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (int i = 0; i < values.size(); i++){
        squaredSum = squaredSum + pow(values[i], 2);
    }
    ...
    for (int i = 0; i < keys.size(); i++){
        newMap.put(keys[i], values[i]/pow(squaredSum, 0.5));
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if (numToKeep < 0){
        error("numToKeep cannot be negative");
    }
    ...
    // Load all keys into the queue and they are from highest to lowest frequency
    for (int i = 0; i < keys.size(); i++){
        queue.enqueue(keys[i], 1 - source[keys[i]]);
    }

    // Add all highest until reach of numToKeep
    for (int i = 0; i < numToKeep; i++){
        string key = queue.dequeue();
        newMap.put(key, source[key]);
    }
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    /* Note to TA: I pick the smaller one to compare against
     * because I was running into efficiency issues earlier
     * (long input test would take too long)*/

    Vector<string> keys = rhs.size() > lhs.size() ? lhs.keys() : rhs.keys();
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    // hSim stores highest similarity
    double hSim = 0;
    string nameHSim = "";
