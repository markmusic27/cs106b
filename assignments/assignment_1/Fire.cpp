...
void updateFire(Grid<int>& fire) {
    const int columns = fire.numCols();
    const int rows = fire.numRows();


    for (int y = 1; y < rows; y++) {
        for (int x = 0; x < columns; x++) {

            // Find cells above that are in bounds (max 3)
            Vector<int> neighbors = {};

            for (int i = -1; i <= 1; i++) {
                if (fire.inBounds(y-1, x+i)) {
                    neighbors.add(x+i);
                }
            }

            // Pick one of the neighbors to copy temp
            int cell = randomElement(neighbors);
            fire[y-1][cell] = fire[y][x];

            if (fire[y-1][cell] != 0 && randomChance(2.0/3.0)){
                fire[y-1][cell]-=1;
            }
