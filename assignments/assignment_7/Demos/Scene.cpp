...
void SceneImpl::drawPoly(const Vector<GPoint>& pts) {
    GPolygon polygon;
    polygon.setColor(canvas()->GDrawingSurface::getColor());

    for (const auto& pt: pts) {
        polygon.addVertex(toScreen(pt));
    }

    canvas()->draw(&polygon);
}

void SceneImpl::fillPoly(const Vector<GPoint>& pts) {
    GPolygon polygon;
    polygon.setFilled(true);
    polygon.setColor(canvas()->GDrawingSurface::getColor());
    polygon.setFillColor(canvas()->GDrawingSurface::getColor());
